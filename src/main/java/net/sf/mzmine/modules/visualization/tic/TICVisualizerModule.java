...
public class TICVisualizerModule implements MZmineRunnableModule {
	...
	public ExitCode runModule(@Nonnull MZmineProject project, @Nonnull ParameterSet parameters,
		...
		final RawDataFile[] dataFiles = parameters.getParameter(TICVisualizerParameters.DATA_FILES).getValue()
		...
		if (weHaveData) {
			TICVisualizerWindow window = new TICVisualizerWindow(dataFiles, plotType, scanSelection, mzRange,
	...
	public static void setupNewTICVisualizer(final RawDataFile[] dataFiles) {
		setupNewTICVisualizer(MZmineCore.getProjectManager().getCurrentProject().getDataFiles(), dataFiles,
				new Feature[0], new Feature[0], null, null, null);
	}

	public static void setupNewTICVisualizer(final RawDataFile[] allFiles, final RawDataFile[] selectedFiles,
			final Feature[] allPeaks, final Feature[] selectedPeaks, final Map<Feature, String> peakLabels,
			ScanSelection scanSelection, final Range<Double> mzRange) {
		...
		if (myParameters.showSetupDialog(null, true, allFiles, selectedFiles, allPeaks, selectedPeaks) == ExitCode.OK) {
			...
			myInstance.runModule(MZmineCore.getProjectManager().getCurrentProject(), p, new ArrayList<Task>());
		}
	}

	public static void showNewTICVisualizerWindow(final RawDataFile[] dataFiles, final Feature[] selectionPeaks,
			final Map<Feature, String> peakLabels, final ScanSelection scanSelection, final TICPlotType plotType,
